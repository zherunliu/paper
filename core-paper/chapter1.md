...
在线强化学习框架中，智能体直接与环境交互，通过试错优化行为策略。该方法具备良好的适应性，能够根据环境反馈及时调整策略，但需要大量交互样本。在医疗诊断\cite{Gottesman2019Healthcare}、自动驾驶\cite{Kiran2021Driving}等数据采集成本高昂或存在安全约束的场景中，频繁的在线试错既不经济也存在风险，限制了在线强化学习的实际部署。
...
上述进展表明强化学习在大模型对齐、表征学习、复杂场景适配等方向取得了实质性成果，但也对离线到在线强化学习技术提出了更高要求。当前该领域仍面临三方面技术挑战：

**分布偏移与知识遗忘问题**。离线阶段基于固定数据集训练的策略，在在线环境中因数据分布差异易产生性能衰减；在线微调过程中又可能丢失离线阶段习得的知识。现有方案通过动态策略调整、重要性加权等机制尝试平衡两者，但需针对具体任务分析数据分布特性并设计回放机制，开发成本较高，且难以在适应新环境与保留已有知识之间取得平衡。

**探索与利用的平衡加剧**。探索-利用权衡是强化学习的固有难题，离线到在线的两阶段模式进一步放大了这一矛盾：离线阶段需保持策略保守性以规避数据偏差风险，在线阶段又需探索未知区域以适配环境变化。两阶段对探索强度的需求相反，使得探索策略的设计更加困难。RT-2模型虽提升了泛化能力，但未形成适配两阶段特性的动态探索机制。

**数据效率与在线开销的失衡**。离线到在线的两阶段设置中，离线数据集的知识常未被充分利用，策略在微调阶段易丢失预训练知识；在线交互虽能获取环境反馈，但频繁访问环境会削弱离线学习低成本的优势。如何在提升数据利用率的同时控制在线开销，是该领域亟待解决的问题。
