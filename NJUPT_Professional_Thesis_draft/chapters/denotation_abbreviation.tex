\begin{denoabbre}
  ...
  \begin{supertabular}{p{4cm}p{9cm}}
    ...
    $K_0$  &  自适应权重的转折点 \\
    $\alpha$  &  状态不确定性与转移不确定性的平衡系数 \\
    $\alpha_{ent}$  &  熵温度参数 \\
    $\text{DisVar}(s,a)$  &  集成模型均值分歧度 \\
    $u_{on}(s,a)$  &  在线阶段综合不确定性 \\
    $c$  &  在线阶段不确定性引导权重 \\
    $c_0$  &  在线阶段初始探索权重 \\
    $k_0,\,\nu$  &  探索权重衰减中心与衰减速率参数 \\
    $N$  &  集成模型成员数量 \\
  ...
  \begin{supertabular}{p{2.5cm}p{8.5cm}p{8cm}}
    ...
    MOReL   &  Model-Based Reinforcement Learning  &  基于模型的强化学习方法 \\
